...
get_anchor_behavior = profile_updater.get_anchor_behavior
...
def compute_relevance_signal(paper: Dict, profile: Dict) -> float:
    ...
    if anchor_topic and anchor_topic in paper_topics:
        anchor_signal = float(anchor_behavior.get("category_bonus", 0.0) or 0.0)

    suppression_penalty = 0.0
    suppressed_topics = [str(topic).strip() for topic in anchor_behavior.get("suppressed_topics", []) or [] if str(topic).strip()]
    if suppressed_topics and not (anchor_topic and anchor_topic in paper_topics):
...
def compute_drift_bonus(paper: Dict, profile: Dict, weights: Dict) -> tuple[float, List[str]]:
    ...
    anchor_topic = str(anchor_behavior.get("target_topic") or "").strip()
    anchor_bonus = 0.0
    matched_anchor_topics: List[str] = []
    if anchor_topic and anchor_topic in paper_topics:
        anchor_bonus = float(anchor_behavior.get("score_bonus", 0.0) or 0.0)
        matched_anchor_topics = [anchor_topic]
    ...
    matched_shift_topics = [topic for topic in paper_topics if topic in top_shift_topics] if status in {"shifting", "recovered"} else []
    short_term_strength = max(float(short_term_topics.get(topic, 0.0) or 0.0) for topic in paper_topics) if short_term_topics else 0.0
    ...
    return round(min(0.20, bonus), 4), list(dict.fromkeys(matched_anchor_topics + matched_shift_topics))[:3]
...
def extract_topics_from_text(title: str, abstract: str = "") -> List[str]:
    ...
    if _title_has_any_token(title_lower, ("reinforcement", "rl", "reward", "policy")):
...
def _title_has_any_token(title: str, tokens: tuple) -> bool:
    """Match short topic tokens as words, so RL does not match world/early/Carlo."""
    normalized_title = f" {re.sub(r'[^a-z0-9]+', ' ', str(title or '').lower()).strip()} "
...
def format_drift_hint(profile: Dict) -> str:
    ...
    if drift_state.get("anchor_topic"):
        anchor_label = format_direction_label(drift_state.get("anchor_topic"))
        progress = float(drift_state.get("anchor_progress", 0.0) or 0.0)
        if int(drift_state.get("commitment_days_remaining", 0) or 0) > 0:
            return f"兴趣迁移状态：已锁定锚点 {anchor_label}，当前正在持续推进（progress={progress:.2f}）。"
        return f"兴趣迁移状态：已形成稳定迁移方向，当前锚点为 {anchor_label}（progress={progress:.2f}）。"
    if drift_state.get("hidden_anchor") and drift_state.get("drift_enabled"):
        anchor_label = format_direction_label(drift_state.get("hidden_anchor"))
        return f"兴趣迁移状态：观察期，系统正在跟踪你是否持续转向 {anchor_label}。"
    if status == "shifting":
...
def daily_push(
    ...
    normalized_profile = ensure_profile_schema(profile)
    initialized_profile = drift_engine.initialize_hidden_anchor(
        normalized_profile,
        strategy_mode=drift_engine.STRATEGY_REAL_USER,
    )
    if initialized_profile.get("drift_state") != profile.get("drift_state"):
        update_profile(user_id, initialized_profile)
