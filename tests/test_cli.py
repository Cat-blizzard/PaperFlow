...
def test_cli_help_lists_all_commands() -> None:
    ...
    assert result.exit_code == 0
    for command in ("init", "doctor", "profile", "daily", "read", "feedback", "gui", "demo", "eval"):
        assert command in result.stdout
    assert "wiki" not in result.stdout.lower()
...
@pytest.mark.unit
def test_cli_rejects_removed_wiki_command() -> None:
    result = runner.invoke(app, ["wiki", "--help"])

    assert result.exit_code == 2
    assert "No such command 'wiki'" in plain_output(result.output)
